...
```hcl
resource "viettelidc_backup_plan" "daily" {
  name             = "daily-backup"
  ...
  number_of_record = 7
  volume_ids       = [viettelidc_volume.data.id]
...
```
terraform import viettelidc_backup_plan.daily <backup_plan_id>
```
