...
```hcl
resource "viettelidc_load_balancer" "web" {
  name              = "web-lb"
  description       = "Load balancer for web tier"
  subnet_id         = viettelidc_subnet.public.id
  floating_ip_id    = viettelidc_floating_ip.fip.id
  ...
  pool_members {
    vm_id  = viettelidc_instance.vm1.id
    port   = 80
    weight = 1
  }

  pool_members {
    vm_id  = viettelidc_instance.vm2.id
    port   = 80
    weight = 1
  }
}
...
```
terraform import viettelidc_load_balancer.web <load_balancer_id>
```
