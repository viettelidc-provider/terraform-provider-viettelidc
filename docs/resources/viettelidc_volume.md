---
layout: "vcloud"
page_title: "Viettel IDC Cloud: viettelidc_volume"
...
Provides a Block Storage Volume on ViettelIDC. Use `viettelidc_volume_attachment` to attach the volume to an instance after creation.

## Example Usage

```hcl
resource "viettelidc_volume" "data" {
  name        = "data-disk"
  size        = 100
  volume_type = "SSD"
...
resource "viettelidc_volume_attachment" "attach" {
  instance_id = viettelidc_instance.vm.id
  volume_id   = viettelidc_volume.data.id
  vpc_id      = viettelidc_vpc.main.id
}
```
...
```
terraform import viettelidc_volume.data <volume_id>
```
