...
```hcl
resource "viettelidc_key_pair" "deploy" {
  key_name = "deploy-key"
  vpc_id   = viettelidc_vpc.main.id
}

output "private_key_url" {
  value     = viettelidc_key_pair.deploy.download_url
  sensitive = true
}
```
...
```
terraform import viettelidc_key_pair.deploy <key_pair_id>
```
