...
```hcl
data "viettelidc_subnets" "all" {
  vpc_id = viettelidc_vpc.main.id
}

output "subnet_ids" {
  value = [for s in data.viettelidc_subnets.all.subnets : s.id]
}
```
