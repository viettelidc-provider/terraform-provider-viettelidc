	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"host_id": schema.Int64Attribute{
				Optional:    true,
				Computed:    true,
				Description: "Host ID (hypervisor cluster). Required by the API to return results (e.g. 6). Falls back to provider config.",
			},
...
func (d *VMTemplatesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if hostID != 0 {
		cfg.HostID = types.Int64Value(hostID)
	} else {
		cfg.HostID = types.Int64Null()
	}
