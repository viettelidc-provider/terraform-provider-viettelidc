...
func ParseAPIResponse(body []byte) (*APIResponse, error) {
	...
	if bodyLower == "success" || bodyLower == `"success"` || bodyLower == "ok" || bodyLower == `"ok"` {
		return &APIResponse{Code: float64(0), Message: "success", Data: json.RawMessage(`{}`)}, nil
	}
	...
	if err := json.Unmarshal(body, &resp); err != nil {
		...
		// If it's a raw non-JSON string, wrap it as a JSON string so it is valid JSON.
		escapedBody, _ := json.Marshal(bodyStr)
		return &APIResponse{Code: float64(0), Data: json.RawMessage(escapedBody)}, nil
	}
	if resp.Code == nil {
		...
		if err2 := json.Unmarshal(body, &errBody); err2 == nil && ((errBody.Success != nil && !*errBody.Success) || len(errBody.Errors) > 0) {
			resp.Code = float64(-1)
